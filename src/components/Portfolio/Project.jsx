...
function Project({ projectDict, firstPortfolio, position }) {
  ...
  const details = <div className="infoCarousel">
    <h2>{projectDict.name}</h2>
    <div className="listInfoProject">
      <ul>
        {projectDict.specifications.map((value, index) =>
          <li className="sublistInfoProject" key={`listInfo-project-${index}`}>{value}</li>
        )}
      </ul>
    </div>
  </div>

  const carousel = <div className={position ? "carouselContainerLeft" : "carouselContainerRight"}>
    <CCarousel controls indicators>
      {projectDict.images.map((image, index) =>
        <CCarouselItem key={`carouselContainer-project-${index}`}>
          <CImage className="d-block w-100 imageCarousel" src={image.img} alt="slide 1" />
  ...
  return (
    <div className={`innerBoxPortfolio ${firstPortfolio ? "" : "firstPortfolio"}`}>
